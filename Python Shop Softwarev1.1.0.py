...
configsec='section_a'
today = date.today()
now = datetime.now().strftime("__%Y_%m_%d-%I_%M_%S_%p")
oldstdout = sys.stdout
readme='readme.txt'
MR='Master_Record.txt'
VN="1.0.1"
...
def createreadme():
        ...
        with open(readme, 'w') as f:
            ...
            print('Changenotes for 1.0.1')
            print('Added Master Record. This will create a "Master Record" file in the /projects directory, and write all jobs to it in addition')
            print('to the individual jobs record. Hopefully, printing support and maybe SQL support will be coming eventually.')
            print('Removed project name from customer-facing record, it will only be used to make a filename and to make a header for the master record.')
            print('Project Name would be a good place to put which bay the work was done in, if you have multiple bays.')
            print('I also set the program up to forcibly rewrite the Readme every time the program is opened.')
...
def getcustinfo():
    ...
    cstinfo = input()
    if cstinfo == 'y' or 'Y' or 'Yes' or 'yes':
            ptf()
    elif cstinfo != 'y':
            getcustinfo()
...
def updatemaster():
    if ((Path.cwd() / 'projects').exists()):
        ...
        os.chdir("projects")
        if os.path.exists(MR)==True:
            print('Master Record found.')
            with open(MR, 'a') as f:
                sys.stdout = f
                print(PNEX)
                ...
                os.chdir('..')
                f.close()
                print('Master Record updated.')
        elif os.path.exists(MR)==False:
            with open(MR, 'w') as f:
                print('Master record not found, creating Master Record')
                f.write('This is the Master Record of all work done by.' +CN)
                f.write('')
                f.write('')
                f.write('/n')
                f.close()
...
def asksettings():
    ...
    print('Sales tax is set to %', Taxpercent)
    print('Shop rate per hour is $', str(shoprate))
    print('Parts Upcharge is set to ', partsuc)
    ...
    if changesettings=="y":
        ...
        print('Current settings are:')
        print('Company name is ' +CN)
        print('Sales tax is set to %', str(float(Taxpercent*100)))
        print('Shop rate per hour is $', str(shoprate))
    ...
    changesettings=input()
    if changesettings=="y" or 'Y' or 'Yes' or 'yes':
        settings()
    elif changesettings=="n" or 'N' or 'No' or 'no':
        main()
    elif changesettings!=("n" or 'N' or 'No' or 'no' or "y" or 'Y' or 'Yes' or 'yes'):
...
def settings():
    ...
    print('press 2 to change Sales Tax, currently %', Taxpercent)
    print('press 3 to change Hourly Shop rate, currently $', str(shoprate))
    print('press 4 to change Technician Name, currently ', str(x))
    print('press 5 to change Upcharge for Parts, currently', str((float(partsuc*100))))
    ...
    if sn=="1":
        ...
        print('Please enter new sales tax percentage as a decimal.')
        print('Example, 8% tax would be entered as .08')
        Taxpercent = input()
